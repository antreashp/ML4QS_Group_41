...
class DistributionBasedOutlierDetection:
    ...
    def mixture_model(self, data_table, col, component, iter_n):

        print('Applying mixture models')
        # Fit a mixture model to our data.
        data = data_table[data_table[col].notnull()][col]
        g = GaussianMixture(n_components=component, max_iter=iter_n, n_init=1)
        ...
        data_probs = pd.DataFrame(np.power(10, probs), index=data.index, columns=[col+'_mixture_'+str(component)+'_'+str(iter_n)])
...
class DistanceBasedOutlierDetection:
    ...
    def local_outlier_factor(self, data_table, cols, d_function, k):
        ...
        data_outlier_probs = pd.DataFrame(outlier_factor, index=new_data_table.index, columns=['lof'+str(k)])
